...
br2.cleaned <- R2(modelbos.cleaned.pred, bted$medv)
brmse.cleaned
br2.cleaned
```
...
R-Squared: interval -> +/ - 1 +- 1 is the max or min. Closer to 1 the better it is. It measures how close the actual data to the regression line. 
If we have 1 that means all the actual data are on exactly lies on the regression line. 
...
Mean Absolute error (MAE): it is similar with RMSE. It's measures the prediction error. 
...
Additionally, we have AIC, AICc, BIC, and Mallows Cp, that are commonly used for model evaluation and selection. Those are unbiased estimate of the model prediction error MSE. We want small # from those result. 
...
AICc: is a verion of AIC corrected for a small sample sizes.
...
AIC() and BIC() computes the AIC and BIC repsectively.
...
models1.R2 <- R2(models1.pred, swiss$Fertility)
...
We have a better R^2 from model2.performance, which means models2 explain better for the predicted value, the model from models2 has a better accuracy. 
...
Cross validation is also known as resampling method
...
swiss.performance <- c(R2(swiss.pred, test.data.swiss$Fertility),
...
4) get the overall prediction error by taking the average of all these test error estimates recorded at step 2
...
In this section, we have go thru 4 different moethods for assessing the performance of a model on unseen test data. 
...
Bootstrap resampling consist of repeatedly selecting a sample of n observations from the original data set and evaluate the model on each copy. An average standard error is then calculated with the results provide an indication of the overall variance of the model performance.
...
2) pply the function boot_fun() to the full data set of 47 observations in order to compute the coefficients
...
Let use boot() function to compute the standard error of 500 bootstrap estiamtes for the coefficients 
```{r}
library(boot)
...
The bootstrap approach does not rely on any of these assumptions made by the linear model, and so it is likely giving a  more accurate estimate of the coefficients and standard erros than the summary function. 
